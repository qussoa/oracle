...
-- 이때 별도의 CONSTRAINT 추가 방식으로 지정해야한다
-- 이 방식은 다수의 칼럼으로 PK를 지정할떄도 사용한다
...
*/
 ...
 /*
    성적테이블을 각 과목이름으로 칼럼을 만들어서 생성을 하면
    데이터를 추가하거나, 단순 조회를 할때는 상당히 편리하게 사용할 수 있다
    그러나 사용중에 과목이 추가 또는 과목명이 변경되는 경우 테이블의 칼럼을
    변경해야 하는 상황이 발생
    테이블의 칼럼을 변경하는 것은 DBMS 입장에서나 사용자 입장에서
    많은 비용(사용중지 시간, 노력, 위험성 등) 지불해야한다
    테이블의 칼럼을 변경하는 일은 매우 신중해야한다
    
    그래서 실제 데이터는 고정된 칼럼으로 생성된 테이블에 저장을 하고
    VIEW로 확인을 할때 PIVOT 방식으로 펼쳐보면 실제 테이블에 칼럼이 존재하는 것처럼
    사용할수 있는 장점이 있다
*/
...
-- SQL Developer에서 명령수행 제한이 있다
-- main from 절에 SUB QUERY를 사용해서 테이블을 지정
SELECT * FROM
( SELECT s_std, s_subject, s_score FROM tbl_score)
    PIVOT (
    SUM(s_score) -- 칼럼이름별로 분리하여 표시할 데이터
    FOR s_subject -- 묶어서 펼칠 칼럼이름 
...
SELECT s_std AS 학생, 
        ...
        SUM(DECODE(s_subject,'수학',s_score)) AS 수학,
        SUM(DECODE(s_subject,'영어',s_score)) AS 영어,
        SUM(s_score) AS 총점, ROUND(AVG(s_score)) AS 평균,
