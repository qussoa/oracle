...
-- 간단한 연산을 SELECT 문을 이용해서 실행할 수 있도록한다
-- SELECT 30+40
-- 오라클에서는 SELECT 다음에 FROM 절이 없으면 문법오류를 발생하도록 설계됨
-- 그래서 표준 SQL에서 사용하는 간단한 코드를 실행하기가 매우 까다로움
-- 문법오류를 방지하고 간단한 코드를 실행하도록 하기 위해서 DUAL 이라는 DUMMY TABLE 준비

SELECT * FROM DUAL;
SELECT '1' AS num, '홍길동'  AS name, '컴공과' AS dept from dual;

-- 위의 dual table 이용한 간단한 코드를 실제 데이터가 있는 table을 상대로 실행하면 table에
-- 저장된 데이터 개수만큼 반복되어서 실행된다
-- 그래서 1개의 레코드만 가진 dummy table을 준비해두고 
-- 이 테이블을 활용하여 코드를 실행하도록하는 것
SELECT '1' AS num, '홍길동'  AS name, '컴공과' AS dept from tbl_student;

-- 가상의 데이터를 하나 만들어보기
-- UNION
-- 여러 테이블을 SELECT해서 생성된 VIEW결과를 묶어서 마치 하난의 결과처럼 보고자 할때 사용
SELECT '1' AS num, '홍길동'  AS name, '컴공과' AS dept from dual
...
-- 실제 사용하는 DBMS 물리적인 테이블을 생성하지 않고 가상의 데이터를 만들어서 사용하고자 할때
-- 임시로 TABLE 구조와 데이터를 생성하여 테스트 용도로 사용하는 명령
-- 이 명령을 사용할 떄 UNION 키워드를 사용
...
-- to_char(값 형식)
-- 숫자형 자료를ㄹ 문자열형 자료로 변환시키는 cascading 함수
-- 형식
-- 9 : 숫자 자릿수를 나타내는 형식 실제 출력되는 값의 자릿수만큼ㅂ 개수를 지정해야한다
--    to_char(123.'99999')  -> 123
-- 0 : 숫자자리수를 나타내는 형식, 실제 출력되는 값보다 자릿수가 많으면 나머지를 0으로 채움
--     to_char(123455,''9,999,999') -> 1,234,567
-- , : 천단위 구분기호
...
UNION ALL SELECT '2' AS num, '성춘향' AS name from dual
UNION ALL SELECT '3' AS num, '이몽룡' AS name from dual
...
SELECT * FROM tbl_student
WHERE st_name IN (SELECT * FROM tbl_temp);

/*
    WHERE 절에 포함된 SELECT 실행 : SELECT st_name FROM tbl_temp
    '기은성', '남동예', '내세원', '갈한수', '방채호', '배채호' 리스트를 생성하고
    WHERE IN('기은성', '남동예', '내세원', '갈한수', '방채호', '배채호') 코드를 생성한다
    
    그리고 이 WHERE 절을 사용해서
    tbl_student 데이터를 조회한다
*/

-- 2. FROM 절에 포함되는 SUBQ 인라인뷰라고 하며 다른 TABLE의 결과를 FROM절에서 사용하는 것
-- 여러 table을 결합하여 나오는 결과값들을 모아사 하나의 쿼리로 연결하여
-- view로 보여주기 위한 SQUERY
-- EQ JOIN 을 대신해서 사용하기도 한다
SELECT * FROM 
...
-- 나. tbl_student 의 st_num 칼럼 값을 sub 쿼리에 전달
--     subq는 마치 method처럼 작동
-- 다. subq에서 where절을 실행할여 데이터 추출
-- 라. SUM(s_kor+s_eng+s_math)를 실행하여
-- 마. 결과를 return
-- 바. main 에서 st_num, 리턴받은 결과 형식으로 표시
